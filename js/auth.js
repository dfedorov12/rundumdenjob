...
/* Anmeldung – OAuth2 Authorization Code Flow mit PKCE (ohne MSAL-Bibliothek),
   identisch zum Orgchart, ergänzt um eine automatische Anmeldung:
...
const AUTH = (() => {
  ...
  function saveTok(t, exp) { _tok = t; _exp = exp; ss.set("rudj_t", t); ss.set("rudj_e", String(exp)); }

  function loadTok() {
    const t = ss.get("rudj_t"), e = +ss.get("rudj_e");
  ...
  async function handleRedirect() {
    ...
    if (p.get("state") !== ss.get("rudj_ps")) {
      history.replaceState({}, document.title, location.pathname);
      return { error: "Ungültiger State – bitte Seite neu laden." };
    }
    const v = ss.get("rudj_pv");
    if (!v) {
      history.replaceState({}, document.title, location.pathname);
